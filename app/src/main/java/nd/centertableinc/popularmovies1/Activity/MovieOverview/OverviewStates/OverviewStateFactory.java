        ...
        switch (state)
        {
            case POPULAR_STATE:
                if(popularState == null)
                    popularState = new PopularMovies(context, listener, POPULAR_STATE);

                ret = popularState;
                break;
            case HIGHEST_RATED_STATE:
                if(highestRatedState == null)
                    highestRatedState = new HighestRatedMovies(context, listener, HIGHEST_RATED_STATE);

                ret = highestRatedState;
                break;
            case FAVORITE_STATE:
                if(favoriteState == null)
                    favoriteState = new FavoriteMovies(context, listener, FAVORITE_STATE);
